...
# config["data"]["samples"] is now a list of file paths
sample_paths = config["data"]["samples"]

# Derive sample names from the file names (without extension)
sample_names = [os.path.splitext(os.path.basename(p))[0] for p in sample_paths]

# Build the samples DataFrame: sample name → input_file path
samples = pd.DataFrame({
    "sample": sample_names,
    "input_file": sample_paths
...
logger.info("    Config files:       " + ", ".join(str(p) for p in workflow.configfiles))
