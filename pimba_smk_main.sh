...
if [ "$run_mode" != "no" ]; then
    if [[ "$run_mode" == *"NCBI"* ]]; then
        ...
        if [ "$remote" == "yes" ]; then
            snakemake --snakefile workflow/Snakefile_run --use-singularity --configfile "$config_file" --cores "$threads" --singularity-args "-B $taxdump:/taxdump -B $taxdump -B $config_file"
        else
            snakemake --snakefile workflow/Snakefile_run --use-singularity --configfile "$config_file" --cores "$threads" --singularity-args "-B $ncbi_db -B $taxdump:/taxdump -B $taxdump -B $config_file"
        fi
    else
...
if [ "$plot_mode" == "yes" ]; then
    ...
    snakemake --snakefile workflow/Snakefile_plot --use-singularity --configfile "$config_file" --cores "$threads" --singularity-args "-B $metadata -B $config_file"
else
